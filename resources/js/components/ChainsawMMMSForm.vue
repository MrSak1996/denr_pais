...
<template>
    <div class="space-y-6">
        <div style="height:350px; overflow-y: auto;">
            <div v-for="(brand, bIndex) in brands" :key="bIndex" class="bg-white border rounded-lg shadow-sm p-5 space-y-4">
                <!-- BRAND HEADER -->
                <Button icon="pi pi-times" severity="danger" text class="mt-3 bg-red-900 hover:bg-red-700 self-start"
                    @click="removeBrand(bIndex)" v-if="brands.length > 1">
                    <Trash2 :size="15" />
                </Button>
                <div class="grid grid-cols-1 md:grid-cols-1 gap-3">
                    <FloatLabel class="mt-3">
                        <InputText v-model="applicationData.supplier_name" v-letters-numbers-dash-uppercase
                            class="w-full" />
                ...
                <DataTable :value="brand.models" responsiveLayout="scroll" class="border rounded-lg overflow-hidden">
                    ...
                    <Column header="Model" :headerStyle="{ backgroundColor: '#0D47A1', color: '#fff', fontWeight: 'bold' }">
                        <template #body="{ data }">
